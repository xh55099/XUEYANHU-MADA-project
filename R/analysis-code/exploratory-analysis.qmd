...
for (i in 1:length(predictors)) {
  subsets <- combn(predictors, i)
  for (j in 1:ncol(subsets)) {
    predictors_subset <- subsets[, j]
    
    # Fit model using subset of predictors
    formula <- paste("BMI ~", paste(predictors_subset, collapse = " + "))
    ...
    # Calculate RMSE
    RMSE1 <- sqrt(mean((testData$BMI - predictions)^2))
    
    # Update minRMSE and bestSubset if RMSE is lower
    if (RMSE1 < minRMSE1) {
      minRMSE1 <- RMSE1
      bestSubset1 <- predictors_subset
    }
  }
}
...
for (i in 1:length(predictors_updated)) {
  ...
  for (j in 1:ncol(subsets)) {
    ...
    # Calculate RMSE
    RMSE2 <- sqrt(mean((testData$BMI - predictions)^2))
    
    # Update minRMSE and bestSubset if RMSE is lower
    if (RMSE2 < minRMSE2) {
      minRMSE2 <- RMSE2
      bestSubset2 <- predictors_subset
    }
  }
}
...
lasso_model <- cv.glmnet(x, y, alpha = 1, nfolds = 10)  # Use alpha = 1 for LASSO regression

# Extract optimal lambda value
best_lambda <- lasso_model$lambda.min

# Extract coefficients of predictors for the optimal lambda
coef <- coef(lasso_model, s = best_lambda)

# Identify non-zero coefficients (selected predictors)
selected_predictors1 <- rownames(coef)[coef != 0]

# Print selected predictors
print(paste("Selected predictors:", paste(selected_predictors1, collapse = ", ")))
```


```{r}
# Fit LASSO regression model using cross-validation with interaction terms
x <- model.matrix(as.formula(paste(response, "~", paste(predictors_updated, collapse = "+"))), data = mydata)[, -1]  # Exclude intercept column
y <- mydata[, response]
lasso_model <- cv.glmnet(x, y, alpha = 1, nfolds = 10)  # Use alpha = 1 for LASSO regression
...
# Extract coefficients of predictors for the optimal lambda
coef <- as.matrix(coef(lasso_model, s = "lambda.min"))

# Identify non-zero coefficients (selected predictors)
selected_predictors <- rownames(coef)[coef != 0]

# Print selected predictors
print(paste("Selected predictors (including interactions):", paste(selected_predictors, collapse = ", ")))
```
...
formula <- as.formula(paste(response, "~", paste(c(predictors, interaction_terms), collapse = " + ")))
